    ...
    if (effectiveAggregationLevel !== undefined && effectiveAggregationLevel !== null) {
      ...
      if (record.is_aggregated === true) {
        ...
        if (selectedLevel1Segments.length > 0) {
          // SPECIAL CASE: For regional segment types ("By Region", "By State", "By Country"),
          // the selected "segment" could be:
          // 1. A top-level geography name (e.g., "Red Sea", "North America")
          // 2. A country name in segment_hierarchy.level_1 (e.g., "Jordan", "Egypt")
          // 3. A port/city name in segment (e.g., "Aqaba", "Safaga")
          if (isRegionalSegmentType) {
            const standardRegionalGeographies = ['North America', 'Europe', 'Asia Pacific', 'Latin America', 'Middle East', 'Africa', 'Global']

            // Check different matching strategies:
            // 1. Selected segment matches the top-level geography (like "Red Sea")
            const selectedMatchesTopLevel = selectedLevel1Segments.includes(record.geography)

            // 2. Selected segment matches standard regional geographies
            const selectedAreStandardRegions = selectedLevel1Segments.some(seg => standardRegionalGeographies.includes(seg))

            // 3. Selected segment matches the hierarchy level_1 (country level, like "Jordan", "Egypt")
            const selectedMatchesLevel1 = selectedLevel1Segments.some(seg =>
              record.segment_hierarchy?.level_1 === seg
            )

            // 4. Selected segment matches the actual segment (port/city level, like "Aqaba", "Safaga")
            const selectedMatchesSegment = selectedLevel1Segments.some(seg =>
              record.segment === seg
            )

            // Check if any matching strategy succeeds
            const belongsToSelection = selectedMatchesTopLevel ||
                                       (selectedAreStandardRegions && selectedLevel1Segments.includes(record.geography)) ||
                                       selectedMatchesLevel1 ||
                                       selectedMatchesSegment
...
export function prepareIntelligentMultiLevelData(
  ...
  records.forEach(record => {
    ...
    // 1. Geography names (e.g., North America, Asia Pacific, Red Sea) - Level 1 selections
    // 2. Country/state names (e.g., U.S., Canada, Germany, Jordan, Egypt) - Level 2+ selections
    // 3. Port/city names (e.g., Aqaba, Safaga) - leaf level selections
    if (isRegionalSegmentType && hasExplicitLevel1Selection) {
      const standardRegionalGeographies = ['North America', 'Europe', 'Asia Pacific', 'Latin America', 'Middle East', 'Africa', 'Global']

      // Get the top-level geography from the record (e.g., "Red Sea")
      const topLevelGeography = record.geography

      // Check different matching strategies:
      // 1. Selected segment matches the top-level geography (like "Red Sea")
      const selectedMatchesTopLevel = selectedLevel1Segments.includes(topLevelGeography)

      // 2. Selected segment matches standard regional geographies
      const selectedAreStandardRegions = selectedLevel1Segments.some((seg: string) => standardRegionalGeographies.includes(seg))

      // 3. Selected segment matches the hierarchy level_1 (country level, like "Jordan", "Egypt")
      const selectedMatchesLevel1 = selectedLevel1Segments.includes(record.segment_hierarchy?.level_1)

      // 4. Selected segment matches the actual segment (port/city level, like "Aqaba", "Safaga")
      const selectedMatchesSegment = selectedLevel1Segments.includes(record.segment)

      if (selectedMatchesTopLevel) {
        // User selected the top-level geography (e.g., "Red Sea") - include all records
        key = topLevelGeography
      } else if (selectedAreStandardRegions && selectedLevel1Segments.includes(record.geography)) {
        // Standard regional geography selection (North America, Europe, etc.)
        key = record.geography
      } else if (selectedMatchesLevel1) {
        // User selected a country (level_1, like "Jordan", "Egypt")
        // Group all ports/cities under this country together
        key = record.segment_hierarchy.level_1
      } else if (selectedMatchesSegment) {
        // User selected a specific port/city (like "Aqaba", "Safaga")
        key = record.segment
      } else {
